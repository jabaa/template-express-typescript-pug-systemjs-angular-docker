...
gulp.task('build.prod', [
  'build.client.copy',
  'build.client.dev.ts',
  'build.client.dev.pug',
  'build.client.dev.scss',
  'build.server',
  'build.config'
]);
...
gulp.task('build.client.copy', () => {
  const src = [
    path.join(config.CLIENT_SRC, '**', '*'),
    '!' + path.join(config.CLIENT_SRC, '**', '*___jb_tmp___'),
...
gulp.task('watch.build.dev', () => {
  watch(
    path.join(config.CLIENT_SRC, '**', '*.ts'),
    (event) => runSequence('build.client.dev.ts', () => browserSync.reload(event.path)));
  watch(
    path.join(config.CLIENT_SRC, '**', '*.scss'),
    (event) => runSequence('build.client.dev.scss', () => browserSync.reload(event.path)));
  watch(
    path.join(config.CLIENT_SRC, '**', '*.pug'),
    (event) => runSequence('build.client.dev.pug', () => browserSync.reload(event.path)));
  const src = [
    path.join(config.CLIENT_SRC, '**', '*'),
    '!' + path.join(config.CLIENT_SRC, '**', '*___jb_tmp___'),
    '!' + path.join(config.CLIENT_SRC, '**', '*.ts'),
